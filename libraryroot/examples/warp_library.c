...
int main(int argc, char *argv[])
{
    ...
    for (i=1; i<argc; i++)
    {
        ...
        {
            printf("warp: -window is not exported by this minigl.library build\n");
            return 20;
        }
        if (0 == local_stricmp(argv[i], "-texture"))
        {
            i++;
            filename = argv[i];
        }
        if (0 == local_stricmp(argv[i], "-flare"))
        {
            i++;
            flarename = argv[i];
        }
        ...
        {
            i++;
            if (0 == local_stricmp(argv[i], "manual"))
            {
                LockMode = MGL_LOCK_MANUAL;
            }
            else if (0 == local_stricmp(argv[i], "auto"))
            {
                LockMode = MGL_LOCK_AUTOMATIC;
            }
            else if (0 == local_stricmp(argv[i], "smart"))
            {
    ...
    {
        printf("warp: could not open minigl.library\n");
        return -1;
    }
    ...
    {
        printf("warp: mglCreateContext failed\n");
        MiniGLClose();
        return -2;
    }
