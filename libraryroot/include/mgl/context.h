...
struct GLcontext_t
{
 ...
 //Surgeon: minimize color-update calls
	...
	*/
	...
	/* MGLV3D_ENABLE_DOUBLE_BUFFER + MGLV3D_DOUBLE_BUFFER_HOLD_LOCK only
	 * (context.c) -- both buffers' LockBitMapTags handles/addresses,
	 * queried once at open and cached, instead of re-locking every
	 * MGLSwitchDisplay. Unused (zeroed) in every other build config. */
	APTR                    bitmapLocks[2];
	ULONG                   vmembases[3];   /* [2] used by MGLV3D_DOUBLE_BUFFER_TRIPLE_NOSYNC only */

	/* MGLV3D_ENABLE_DOUBLE_BUFFER + MGLV3D_DOUBLE_BUFFER_SAFEMSG only
	 * (context.c) -- the REAL AmigaOS double-buffer synchronization
	 * protocol (graphics.library/AllocDBufInfo autodoc's own reference
	 * algorithm), replacing the mn_ReplyPort=NULL/busy-poll pattern every
	 * other double-buffer variant uses. dbufSafePort receives a message
	 * when the buffer just flipped AWAY from is safe to write into again;
	 * dbufDispPort receives one when it's safe to call ChangeScreenBuffer
	 * again (confirms the last flip was actually displayed at least once).
	 * dbufSafeToWrite/dbufSafeToChange track whether a wait is currently
	 * owed, matching the autodoc example's SafeToWrite/SafeToChange
	 * flags exactly. */
	struct MsgPort *        dbufSafePort;
	struct MsgPort *        dbufDispPort;
	GLboolean               dbufSafeToWrite;
	GLboolean               dbufSafeToChange;

	/* MGLV3D_DOUBLE_BUFFER_VPBITMAP only -- raw graphics.library double
	 * buffering (AllocDBufInfo/ChangeVPBitMap), bypassing Intuition's
	 * AllocScreenBuffer/ChangeScreenBuffer/struct ScreenBuffer entirely --
	 * a genuine pointer-swap (ChangeVPBitMap reassigns the ViewPort's
	 * bitmap pointer directly), not a blit/copy. dbufBitMaps[0] is the
	 * screen's own already-allocated bitmap (freed when the screen
	 * closes, not by us); dbufBitMaps[1] is a plain AllocBitMap'd extra
	 * buffer we own. dbufInfo is ONE shared DBufInfo for both (per the
	 * graphics.library/AllocDBufInfo autodoc's own reference example --
	 * a single persistent DBufInfo tracks current-vs-previous across
	 * calls; MGLV3D_DOUBLE_BUFFER_SAFEMSG's bug was giving each buffer
	 * its own separate DBufInfo instead, via Intuition's per-ScreenBuffer
	 * sb_DBufInfo). Reuses dbufSafePort/dbufDispPort/dbufSafeToWrite/
	 * dbufSafeToChange above -- same message protocol, different (lower-
	 * level) buffer-swap primitive underneath it. */
	struct BitMap *         dbufBitMaps[2];
