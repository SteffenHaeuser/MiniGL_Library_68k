...
#define DUMP_VERTEX(vert) \
 mykprintf("x:%6.3f y:%6.3f z:%6.3f w:%6.3f\nR:%6.3f G:%6.3f B:%6.3f A:%6.3f\nU:%6.3f V:%6.3f\noutcode=0x%X\n",\
		 ...
		 (vert).v.u0, (vert).v.v0, (vert).outcode)
...
static void hc_ClipWZero(MGLVertex *a, MGLVertex *b, MGLVertex *r, GLenum shading)
{
	...
	{
	r->color.a = LERP(t,a->color.a, b->color.a);
	r->color.r = LERP(t,a->color.r, b->color.r);
	r->color.g = LERP(t,a->color.g, b->color.g);
	r->color.b = LERP(t,a->color.b, b->color.b);
	}
	r->v.u0 = LERP(t,a->v.u0, b->v.u0);
	r->v.v0 = LERP(t,a->v.v0, b->v.v0);

	if(mini_CurrentContext->Texture2D_State[1])
	{
		r->v.u1 = LERP(t,a->v.u1, b->v.u1);
		r->v.v1 = LERP(t,a->v.v1, b->v.v1);
	}
...
/*
NEGW is not recomputed: the NEGW pass runs before this one.
*/
...
/*
NEGW, LEFT and RIGHT are not recomputed: those passes run before this one.
*/
...
/*
Surgeon: BUGFIX
These expressions test the length of the triangle-sides and is in essence a simple areatest - it should return GL_FALSE instead of GL_TRUE
*/
...
#if 0
//this test rarely catches a triangle, so it is disabled anyway
...
void hc_ClipAndDrawPoly(GLcontext context, MGLPolygon *poly, ULONG or_codes)
{
	...
	**
	** This clipping algorithm sucessively clips against the w = CLIP_EPS
	** plane and then each of the six clipping planes.
	...
	** Case 1: Edge is completely inside -> one output
